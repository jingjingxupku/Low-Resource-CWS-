...
namespace Program
{
    ...
    class TrainThread
    {
        ...
        public double [,] biacc=new double[4,4];
        public double[,] biaccup = new double[4, 4];
        public double[,] biaccdown = new double[4, 4];
        ...
        public void run(object param)
        {
            ...
            for(int i=0;i<x1.Count;i++)
            {
                ...
                //List<Matrix> returnObj1 = Global._LSTMLayer.activate(x, g);
                //List<Matrix> returnObj7 = Global._LSTMLayerr.activate(reverse(x), g);

                List<Matrix> add = new List<Matrix>();
                for (int k = 0; k < x1[i].inputs.Count(); k++)
                {
        ...
        {
            param pa = (param)param;
            int sentenceindex = pa.sentenceindex;
            List<DataStep> x1 = pa.datastep;
            ...
            int [] relationindex=new int[x1.Count];
            ...
            while (index < x1.Count())
            {
                ...
                for (int i = 0; i < temp.Count; i++)
                {
                    ...
                    for (int k = 0; k < temp[i].inputs.Count(); k++)
                    {
                        add.Add(Global.wordEmbedding[temp[i].inputs[k]]);
                    }
                    List<Matrix> returnObj2 = Global._GRNNLayer1.activate(add, g);
           ...
           fscore.backprocess(wordindeis, ires4,igold4);
